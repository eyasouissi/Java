package esprit.services.group;

import esprit.tools.MyDataBase;
import jakarta.persistence.EntityManager;
import esprit.entities.group.GroupStudent;
import esprit.entities.project.Project;
import esprit.services.IServices;
...
public class GroupService implements IServices<GroupStudent> {
    ...
    public void ajouter(GroupStudent group) {
        ...
        try (PreparedStatement pst = cnx.prepareStatement(query, Statement.RETURN_GENERATED_KEYS)) {
            ...
            for (Project project : group.getProjects()) {
                ...
                Project existingProject = projectService.findByTitle(project.getTitre());
                if (existingProject == null) {
                    ...
                    try (PreparedStatement updateStmt = cnx.prepareStatement(updateProjectQuery)) {
                        updateStmt.setLong(1, group.getId());
                        updateStmt.setLong(2, existingProject.getId());
                        updateStmt.executeUpdate();
                    }
                }
            }
