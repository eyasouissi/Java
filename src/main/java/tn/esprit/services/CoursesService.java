...
public class CoursesService implements IServices<Courses> {
    ...
    public void supprimer(int id) {
        ...
        String deleteRatingsQuery = "DELETE FROM ratings WHERE course_id = ?";
    ...
    public double getAverageRating(int courseId) {
        String query = "SELECT AVG(rating) as average_rating FROM ratings WHERE course_id = ?";
        try (PreparedStatement pst = cnx.prepareStatement(query)) {
            pst.setInt(1, courseId);
            ResultSet rs = pst.executeQuery();
            if (rs.next()) {
    ...
    public int getPopularCount() {
        ...
        String query = "SELECT COUNT(*) FROM courses c " +
                "JOIN (SELECT course_id, AVG(rating) as avg_rating FROM ratings GROUP BY course_id) r " +
