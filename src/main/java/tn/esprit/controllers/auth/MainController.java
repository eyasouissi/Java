...
import tn.esprit.controllers.frontOffre;
import tn.esprit.controllers.user.ProfileController;
import tn.esprit.controllers.user.admin.AdminProfileController;
import tn.esprit.controllers.frontOffre; // You'll need to create this controller
import tn.esprit.entities.User;
import tn.esprit.utils.UserSession; // Create this utility class
...
public class MainController {
    ...
    static {
        ...
        FXML_PATHS.put("Offers", "/interfaces/offres/liste_offres.fxml"); // Add new path
    }
    ...
    // New navigation method for offers
    @FXML

    private void navigateToOffers(javafx.event.ActionEvent event) {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/interfaces/frontOffre.fxml"));
            Parent root = loader.load();

            // Make sure the currentUser is stored in the UserSession
            if (currentUser != null) {
                UserSession.getInstance().setCurrentUser(currentUser);
            }

            // Pass the user to the frontOffre controller
            frontOffre controller = loader.getController();
            if (controller != null) {
                User user = (currentUser != null) ? currentUser : UserSession.getInstance().getCurrentUser();
                controller.setCurrentUser(user);
                System.out.println("User passed to frontOffre: " + (user != null ? user.getName() : "null"));
            }
            ...
            e.printStackTrace();
            showAlert("Error", "Failed to navigate to Offers: " + e.getMessage());
        }
    }
    ...
    private void loadContent(String viewName) {
        try {
            ...
            if (fxmlPath != null && contentArea != null) {
                ...
                // For the offers view, initialize the controller with current user
                if ("Offers".equals(viewName)) {
                    try {
                        frontOffre controller = loader.getController();
                        controller.setCurrentUser(currentUser);
                    } catch (Exception e) {
                        System.err.println("Error initializing OffresController: " + e.getMessage());
                    }
                }
